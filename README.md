...
| 桿頭追蹤 | YOLO11n 桿頭偵測；沒有模型或模型沒抓到時，用影像偵測細直線找桿身（不需訓練，允許偏離雙手中心，並排除靜態背景線），再沿桿身逐段追蹤到末端（以線段平均對比抵抗草地／墊子紋理，並逐步修正角度），於末端附近找桿頭區塊中心。每格保留多個候選，以動態規劃從整段影片挑出最連續的路徑，連續性同時考慮「桿身跟著手臂轉」與「桿身滯後」兩種運動；再用等加速度模型做前向卡爾曼加反向 RTS 平滑（前後慣性），推估缺漏格並剔除殘差過大的量測 |
