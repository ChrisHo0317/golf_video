...
| 桿頭追蹤 | YOLO11n 桿頭偵測（已內建 v1 模型，以 15 支揮桿、逾千格人工標記訓練）；模型沒抓到時，用影像偵測細直線找桿身（不需訓練，允許偏離雙手中心，並排除靜態背景線），再沿桿身逐段追蹤到末端（以線段平均對比抵抗草地／墊子紋理，並逐步修正角度），最後以區域分割（門檻取自桿身自身對比，並依明暗方向排除旁邊的球）取桿頭中心。每格保留多個候選，以動態規劃從整段影片挑出最連續的路徑，連續性同時考慮「桿身跟著手臂轉」與「桿身滯後」兩種運動；再用等加速度模型做前向卡爾曼加反向 RTS 平滑（前後慣性），推估缺漏格並剔除殘差過大的量測 |
...
以 7 支人工逐格標記的揮桿（1112 格，720×1280）評估桿頭位置誤差（平均／中位數／90 百分位，像素）：

| 版本 | 全部 | 準備 | 上桿 | 下桿 | 收桿 |
|---|---|---|---|---|---|
| v0.0.10（影像桿身偵測） | 58/52/94 | — | — | — | — |
| v0.0.12（沿桿身追蹤 + 區域分割） | 23/8/65 | 22/8/65 | 31/7/93 | 43/4/203 | 14/6/28 |
| v0.0.13（YOLO11n 桿頭模型 v1） | **6/3/9** | 5/3/8 | 7/4/15 | 7/3/12 | 12/5/26 |
