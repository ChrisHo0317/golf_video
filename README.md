...
| 桿頭追蹤 | YOLO11n 桿頭偵測；沒有模型或模型沒抓到時，用影像偵測細直線找桿身（不需訓練，允許偏離雙手中心，並排除靜態背景線）。每格保留多個候選，以動態規劃從整段影片挑出最連續的路徑，連續性同時考慮「桿身跟著手臂轉」與「桿身滯後」兩種運動；再用等加速度模型做前向卡爾曼加反向 RTS 平滑（前後慣性），推估缺漏格並剔除殘差過大的量測 |
| 揮桿階段 | 自動切分準備、起桿、頂點、擊球、收桿，可手動修正 |
| 疊加圖層 | 13 種圖層可個別開關並調整顏色與透明度：骨架、桿頭軌跡（依速度著色）、桿身線、手部軌跡、頭部位移、臀部位移、脊椎角度、揮桿平面、肩線、髖線、階段標記、旋轉儀表、參考格線 |
